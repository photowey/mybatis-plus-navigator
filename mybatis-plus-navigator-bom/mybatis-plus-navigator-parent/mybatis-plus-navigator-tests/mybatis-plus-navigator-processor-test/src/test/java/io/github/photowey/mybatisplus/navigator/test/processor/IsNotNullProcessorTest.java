...
import io.github.photowey.mybatisplus.navigator.test.core.query.EmployeeIsNotNullQuery;
import org.junit.jupiter.api.Assertions;
import org.junit.jupiter.api.Test;
import org.springframework.boot.test.context.SpringBootTest;

/**
 * {@code IsNotNullProcessorTest}
 *
 * @author photowey
 * @version 3.5.5.1.0
 * @since 2024/05/29
...
class IsNotNullProcessorTest extends LocalTest {
    ...
    @Test
    void testIsNotNull() {
        EmployeeIsNotNullQuery query = EmployeeIsNotNullQuery.builder()
                .id(1716914167000L)
                .build();

        QueryWrapper<Employee> wrapper = query.tryVisitQueryWrapper();
        String targetSql = wrapper.getTargetSql();
        Assertions.assertEquals("(id = ? AND organization_id IS NOT NULL)", targetSql);
    }
}